...
AIL.4B: review attempts for the review & retention loop.

Purely additive: one new table, ``review_attempts``, and its indexes. No
existing table is touched, no column is added to one, and no CHECK constraint
is rebuilt (unlike AIL.3C), so the upgrade needs no batch table rebuild.
...
The downgrade refuses, changing nothing, while any review attempt exists,
rather than silently discarding a learner's review history.
"""

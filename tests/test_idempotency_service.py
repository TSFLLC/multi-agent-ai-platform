...
def test_internal_test_job_endpoint_is_idempotent(client, db):
    key = str(uuid.uuid4())

    first = client.post("/internal/test-jobs", headers={"Idempotency-Key": key})
    assert first.status_code == 201
    job_id_1 = first.json()["id"]

    second = client.post("/internal/test-jobs", headers={"Idempotency-Key": key})
...
def test_internal_test_job_endpoint_without_key_creates_distinct_jobs(client, db):
    first = client.post("/internal/test-jobs")
    second = client.post("/internal/test-jobs")

    assert first.json()["id"] != second.json()["id"]
    assert db.query(JobQueue).count() == 2


def test_internal_test_job_get_returns_current_status(client, db):
    repo = JobQueueRepository()
    job = repo.enqueue(db, job_type=JobType.INTERNAL_TEST, payload_ref="direct-1")

    resp = client.get(f"/internal/test-jobs/{job.id}")
    assert resp.status_code == 200
    assert resp.json()["status"] == "pending"


def test_internal_test_job_get_missing_returns_404(client):
    resp = client.get("/internal/test-jobs/00000000-0000-0000-0000-000000000000")
